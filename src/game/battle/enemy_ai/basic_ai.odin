...
//= Procedures
run_ai :: proc() {
	switch game.battleStruct.enemyMonster.enemyAi {
...
physical_close_range_ai :: proc() {
	...
	attacking := 4
	for i in 0..<4 {
		#partial switch enemy.monsterInfo.attacks[i].type {
			case .tackle:
				if enemy.monsterInfo.attacks[i].cooldown <= 0 {
					if distance(enemy.position + {0.5,0.02,1}, enemy.target) <= 3 {
						...
						if player.position.x > enemy.position.x do movement.x =  0.02 * ((f32(enemy.monsterInfo.spd) / 100) + 1)
						if player.position.x < enemy.position.x do movement.x = -0.02 * ((f32(enemy.monsterInfo.spd) / 100) + 1)
						if player.position.z > enemy.position.z do movement.z =  0.02 * ((f32(enemy.monsterInfo.spd) / 100) + 1)
						if player.position.z < enemy.position.z do movement.z = -0.02 * ((f32(enemy.monsterInfo.spd) / 100) + 1)
						enemy.position += movement
					}
				} else do enemy.monsterInfo.attacks[i].cooldown -= 1
			case .scratch:
